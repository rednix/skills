...
6. **成交时庆祝**：当 `is_deal` 为 true 时，恭喜用户并告知获得的优惠券面额。**成交后不要再发送消息，会话已结束。**
7. **未成交时安慰**：当 `status` 变为 `no_deal` 时，安慰用户并告知本次砍价结束。**不要再发送消息。**
8. **处理错误**：暗号错误返回 403，会话结束返回 400 — 向用户友好地说明情况。
