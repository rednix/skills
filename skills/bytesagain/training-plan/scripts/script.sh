#!/usr/bin/env bash
# training-plan - Productivity and task management tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${TRAINING_PLAN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/training-plan}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
training-plan v$VERSION

Productivity and task management tool

Usage: training-plan <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "training-plan v$VERSION" ;;
